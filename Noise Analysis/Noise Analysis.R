...
time_on=sweeps[90000:490000,1];
p_curr_on=sweeps[90000:490000,sweep_num];
...
base_spline_on=smooth.spline(time_on,p_curr_on,nknots=length(p_curr_on)/100);
...
#Grab power spectra
spec_on=pspectrum(subtracted_on, plot=FALSE,x.frqsamp=20000);
...
time_off=sweeps[550000:950000,1];
p_curr_off=sweeps[550000:950000,sweep_num];
...
base_spline_off=smooth.spline(time_off,p_curr_off,nknots=length(p_curr_off)/100);
...
spec_off=pspectrum(subtracted_off, plot=FALSE,x.frqsamp=20000);
...
plot(y ~ x, data = subtracted_on_df, main="Subtracted Current Trace - On", type="l",xlim=c(max(time_on)-1.1,max(time_on))-1,ylim=c(-20,20),xlab="Time (s)",ylab="I (pA)")
plot(y ~ x, data = subtracted_off_df, main="Subtracted Current Trace - Off", type="l",xlim=c(max(time_off)-1.1,max(time_off))-1,ylim=c(-20,20),xlab="Time (s)",ylab="I (pA)")
