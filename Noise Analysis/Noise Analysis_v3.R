...
#open file
sweeps=read.csv("ChroME2f_100821_cell4.csv");
...
#for (i in c(2:2)){
  ...
  time_off=sweeps[750000:850000,1];
  p_curr_off=sweeps[750000:850000,sweep_num];
  ...
  spec_off=pspectrum(subtracted_off_limit, plot=FALSE,x.frqsamp=20000,niter=10);
...
#print your results!
dat_data=matrix(nrow=ncol(sweeps)-1,ncol=3)
dat_data[,1]=u_cond;
dat_data[,2]=sub_curr;
dat_data[,3]=dwell_times;
write.csv(dat_data,"ChroME2f_100821_cell1_noise_analysis.csv");
...
plot(y ~ x, data = subtracted_on_df, main="Subtracted Current Trace - On", type="l",xlim=c(min(time_on),min(time_on)+0.01*(max(time_on)-min(time_on))),ylim=c(-2,2),xlab="Time (s)",ylab="I (pA)")
plot(y ~ x, data = subtracted_off_df, main="Subtracted Current Trace - Off", type="l",xlim=c(min(time_off),min(time_off)+0.01*(max(time_off)-min(time_off))),ylim=c(-2,2),xlab="Time (s)",ylab="I (pA)")
