...
def resid(par, x, y, fmin, fmax):
    """
    Calculate the squared error from a Lorentzian model with the given parameters.
...
def plot_my_data(on_time, subtracted_on, off_time, subtracted_off, f_on, f_off, spec_on, spec_off, delta_spec, model_PSD, file_name):
    """
    ...
    on.set(ylabel = "Current (pA)", 
           xlabel = "Time (\u03bcs)",
           title = "On",
           ylim = [min(min(subtracted_on), min(subtracted_off)), max(max(subtracted_on), max(subtracted_off))],
           xticks = [on_time[0],on_time[400],on_time[800],on_time[1200],on_time[1600],on_time[2000]],
    ...
    off.set(ylabel = "Current (pA)", 
           xlabel = "Time (\u03bcs)",
           title = "Off",
           ylim = [min(min(subtracted_on), min(subtracted_off)), max(max(subtracted_on), max(subtracted_off))],
           xticks = [off_time[0],off_time[400],off_time[800],off_time[1200],off_time[1600],off_time[2000]],
           xticklabels = ['0','20','40','60','80','100'])
    ...
    ps.set(ylabel = "PSD, (pA$^2$/Hz)", 
           xlabel = "Frequency (Hz)",
           title = "Power Spectra",
           ylim = [min(min(spec_on[-1000:-1]), min(spec_off[-1000:-1])), max(max(spec_off), max(spec_on))])
...
def ttscna(file_name, on_start, off_start, sweep_len = 5.0, sample_rate = 20000, skip_rows = 9, sep = "\t", fmin = 20.0, fmax = 10000.0):
    """
    Takes as input a text file containing electrophysiological data in two conditions.
    Computes the difference in single-channel noise between the two conditions.
    ...
    # Open file. Note to self: pd.read_table is approx 30X faster than np.load_txt
    ...
    rez = scipy.optimize.leastsq(resid, [1, 1], args = (f_off,delta_spec,fmin,fmax) )[0]
    model_PSD = loren(rez, f_on)

    # Convert the parameters of the Lorentzian to unitary conductances and dwell times.
    # Single-channel Unitary Conductance
    # 1000X because you're converting pS to fS
    u_cond = rez[0] * np.pi * rez[1] / (-0.06 * 2 * (np.mean(on_curr) - np.mean(off_curr))) * 1000
    ...
    # Plot everything!
    plot_my_data(on_time, subtracted_on, off_time, subtracted_off, f_on, f_off, spec_on, spec_off, delta_spec, model_PSD, file_name)

    # Export relevant data to a data frame.
    data = pd.DataFrame({'Unitary Conductance (fS)' : u_cond, 'Dwell Time (ms)' : dwell_time, 'Mean Bulk Current (pA)': sub_curr}, index = [0])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = "TTscna - Single Channel Noise Analysis")
    parser.add_argument('--file', action = 'store', dest = 'file_name',
                        help = 'Name of the file to read, including path and file extension.')
    parser.add_argument('--on', action = 'store', dest = 'on_start',
                        help = 'Start time of the on condition.')
    parser.add_argument('--off', action = 'store', dest = 'off_start',
                        help = 'Start time of the off condition.')
    parser.add_argument('--len', action = 'store', dest = 'sweep_len', default = 5.0,
                        help = 'Length of time, in seconds, of each condition to analyze.')
    parser.add_argument('--rate', action = 'store', dest = 'sample_rate', default = 20000.0,
                        help = 'Sampling rate of the time series, in Hz.')
    parser.add_argument('--skip', action = 'store', dest = 'skip_rows', default = 9,
    ...
    parser.add_argument('--fmin', action = 'store', dest = 'fmin', default = 20.0,
                        help = 'Minimum frequency (in Hz) to fit to.')
    parser.add_argument('--fmax', action = 'store', dest = 'fmax', default = 10000.0,
                        help = 'Maximum frequency (in Hz) to fit to.')
    answer = parser.parse_args()

    # Execute Function!
    print(ttscna(answer.input_string, answer.on_start, answer.off_start, answer.sweep_len, answer.sample_rate, answer.skip_rows, answer.sep, answer.fmin, answer.fmax))
